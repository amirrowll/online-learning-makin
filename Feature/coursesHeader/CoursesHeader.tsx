  ...
  return (
    <div className='mt-28 px-3'>
      <div className='flex items-center justify-between'>
        <h1 className='flex items-center text-[20px] font-kalamehMedium text-[#051116] '><FaSquare className='text-[#F28C28] w-6 h-6' /> همه دوره ها </h1>
        <span className='text-[#8D8D8D] text-[18px] font-kalamehMedium  '> ۶۶ عنوان آموزشی </span>
      </div>
      ...
      <div className='flex items-start mt-8 '>
        <div className='border-[2px] shadow-lg rounded-lg w-fit px-3 py-7 sm:flex sm:flex-col sm:items-center hidden '>
          <label className="flex items-center cursor-pointer">
            <input type="checkbox" value="" className="sr-only peer" checked />
            <span className="text-[14px] font-kalamehMedium text-[#051116] "> دوره های تخفیف دار </span>
            <div className="relative mr-2 w-11 h-6 bg-gray-200 rounded-full peer dark:bg-gray-700 peer-focus:ring-4 peer-focus:ring-orange-300 dark:peer-focus:ring-orange-800 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-[#F28C28] dark:peer-checked:bg-[#F28C28]"></div>
          </label>

          <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-[#051116] flex items-center text-[16px] font-kalamehRegular mt-5" type="button"><LuFolderPlus /> دسته بندی دوره ها <IoIosArrowDown /></button>
        ...
        <div className='flex items-end font-kalamehMedium text-[15px] '>
          <IoSwapVerticalSharp className='w-6 h-6 mx-2' />
          <h1 className='text-[#051116]'> مرتب سازی بر اساس: </h1>
          {sorting.map(item => (
            <ul key={item.id} className='pr-3'>
              <li>{item.text}</li>
            </ul>
          ))}
        </div>
